...
describe("app shell", () => {
  ...
  it("shows the app name and the contest countdown in the header", async () => {
    ...
    expect(within(header).getByRole("link", { name: "SK8 Habits" })).toHaveAttribute("href", "/");
  ...
  it("offers a bottom navigation with all sections", async () => {
    ...
    expect(links.map((link) => link.textContent)).toEqual(["Start", "Heute", "Schlaf", "Knie"]);
  ...
  it("navigates via the bottom navigation and records the screen change", async () => {
    ...
    await user.click(within(nav).getByRole("link", { name: "Schlaf" }));
    expect(await screen.findByRole("heading", { level: 1, name: "Schlaf" })).toBeInTheDocument();
    expect(telemetry.currentScreen).toBe("/sleep");
    ...
    expect(events[1]).toMatchObject({
      screen: "/",
      target: "/sleep",
      meta: { from: "/", to: "/sleep", via: "bottom-nav" },
    });
    expect(events[2]).toMatchObject({ screen: "/", meta: { ms: expect.any(Number) } });
    expect(events[3]).toMatchObject({ screen: "/sleep", meta: { from: "/" } });
  });
